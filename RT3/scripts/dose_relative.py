...
def influence_detecteur():
    ...
    for det in detecteurs:
        for et in etude:
            ...
            plt.figure(figsize=(12, 7))
            plt.plot(df_10.iloc[:, 0], df_10.iloc[:, 1], label='Champ 10x10 cm2')
            plt.plot(df_3.iloc[:, 0], df_3.iloc[:, 1], label='Champ 3x3 cm2')
...
def champs():
    for champ in ['3x3', '6x6', '8x8', '12x12', '15x15', '20x20']:
        ...
        # df_IO = pd.read_excel('../mesures/export_excel/champ_' + champ + '.xlsx', 'Inline X6 Ouverture')
        # df_IF = pd.read_excel('../mesures/export_excel/champ_' + champ + '.xlsx', 'Inline X6 Fermeture')
        plt.plot(df_CO.iloc[:, 0], df_CO.iloc[:, 1], label='Crossline Ouverture')
        plt.plot(df_CF.iloc[:, 0], df_CF.iloc[:, 1], label='Crossline Fermeture')
        # plt.plot(df_IO.iloc[:, 0], df_IO.iloc[:, 1], label='Inline Ouverture')
        # plt.plot(df_IF.iloc[:, 0], df_IF.iloc[:, 1], label='Inline Fermeture')
        plt.grid(ls='--')
        plt.legend(loc='upper right', fontsize=8)
        plt.xlabel('Distance (mm)')
        plt.ylabel('Dose (%)')
